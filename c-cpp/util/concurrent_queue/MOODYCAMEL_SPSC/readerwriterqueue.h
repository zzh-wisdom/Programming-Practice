...
public:
	...
	bool try_dequeue(U& result) AE_NO_TSAN
	{
		...
		non_empty_front_block:
			...
			if (blockFront != blockTail) {  // 额外判断一次当前块是否为空，由于前面已经判断一次 frontBlock_ != tailBlock，生产者后续不会在本块入队
...
private:
...
#endif
	{
		...
		// localFront的影子惰性更新的设计，也减少了原子变量front的竞争访问。
		...
		if (nextBlockTail != blockFront || nextBlockTail != (tailBlock_->localFront = tailBlock_->front.load())) {
			...
			if (tailBlock_->next.load() != frontBlock) {
				...
				newBlock->next = tailBlock_->next.load();  // 这里需要插入一个fence吧，不过只是单生产者和单消费者，问题不大
				tailBlock_->next = newBlock;

				// 这里的意思是，消费者的前进受tailBlock的限制，不会超过tailBlock指向的块。
