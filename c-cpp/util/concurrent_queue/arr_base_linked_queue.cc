...
struct ArrayBaseLinkedQueue::Node {
    const void* value;
    std::atomic<Node*> next;
    Node(ArrayBaseLinkedQueue* q) : value(nullptr), next(nullptr), ref(0), queue(q) {}
  ...
  private:
    friend class ArrayBaseLinkedQueue;
    std::atomic<uint32_t> ref;
    ArrayBaseLinkedQueue* queue;
};

void ArrayBaseLinkedQueue::InitPool(uint32_t size) {
...
ArrayBaseLinkedQueue::Node* ArrayBaseLinkedQueue::AllocNode() {
...
void ArrayBaseLinkedQueue::FreeNode(Node* node) {
...
ArrayBaseLinkedQueue::ArrayBaseLinkedQueue(uint32_t size) {
...
ArrayBaseLinkedQueue::~ArrayBaseLinkedQueue() {
    for (uint32_t i = 0; i < node_arr_.size(); ++i) {
        delete node_arr_[i];
    }
}

ArrayBaseLinkedQueue::Node* ArrayBaseLinkedQueue::SafeRead(std::atomic<Node*> &q) {
    while(true) {
...
class ArrayBaseLinkedQueue::NodeRefGuard {
public:
    NodeRefGuard(ArrayBaseLinkedQueue::Node* node) : node_(node) {}
    ~NodeRefGuard() { node_->DecRef(); }
private:
    ArrayBaseLinkedQueue::Node* node_;
};


bool ArrayBaseLinkedQueue::enqueue(const void* val) {
    Node* n = AllocNode();
    if (!n) return false;
    n->value = val;
    n->next = nullptr;
...
bool ArrayBaseLinkedQueue::dequeue(const void** val) {
    Node* head = nullptr;
    while (true) {
        // 取出头指针，尾指针，和第一个元素的指针
        head = SafeRead(head_); 
...
int ArrayBaseLinkedQueue::size() { return -1; }

int ArrayBaseLinkedQueue::capacity() { return node_arr_.size() - 1; }
